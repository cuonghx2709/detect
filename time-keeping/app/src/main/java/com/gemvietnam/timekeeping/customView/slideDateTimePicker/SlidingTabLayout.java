package com.gemvietnam.timekeeping.customView.slideDateTimePicker;
...
public class SlidingTabLayout extends HorizontalScrollView {
    ...
    private void scrollToTab(int taBindViewex, int positionOffset) {
        final int tabStripChildCount = mTabStrip.getChildCount();
        if (tabStripChildCount == 0 || taBindViewex < 0 || taBindViewex >= tabStripChildCount) {
            return;
        }

        View selectedChild = mTabStrip.getChildAt(taBindViewex);
        if (selectedChild != null) {
            int targetScrollX = selectedChild.getLeft() + positionOffset;

            if (taBindViewex > 0 || positionOffset > 0) {
