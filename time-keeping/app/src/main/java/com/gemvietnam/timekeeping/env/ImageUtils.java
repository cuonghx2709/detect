...
public class ImageUtils {
    ...
    public static Bitmap getFaces(Context context, byte [] ByteArray){
        Bitmap result = null;
        try {
            ByteArrayInputStream imageStrem = new ByteArrayInputStream(ByteArray);
            Bitmap bitmap = BitmapFactory.decodeStream(imageStrem);
            ...
            for(int i = 0; i < faces.size() ;i++){
                ...
                int x2 = x/4 + (int) face.getWidth();
                int y2 = y/4 + (int) face.getHeight();
                result = Bitmap.createBitmap(bitmap, (int)face.getPosition().x, (int)face.getPosition().y, x2, y2);
            }
        } catch (Exception e){
            e.printStackTrace();
        }
    ...
    public static Bitmap getFaces(Context context, int id){
        Bitmap result = null;
        try {
